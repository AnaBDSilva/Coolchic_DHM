...
def sfmr(field, display, roi_coords=None):
    field = np.array(field)
    M, N = field.shape
    
    # 1. FFT
    fft_holo = np.fft.fftshift(np.fft.fft2(field))
    
    # 2. Lógica de Seleção do ROI
    if roi_coords is None:
        # Se não houver coordenadas, fazemos a seleção interativa
        mag = np.abs(fft_holo)
        fft_display = 20 * np.log1p(mag)
        fft_min, fft_max = fft_display.min(), fft_display.max()
        fft_norm = (255 * (fft_display - fft_min) / (fft_max - fft_min)).astype(np.uint8)

        win_name = "Selecione a ROI (Enter para confirmar)"
        cv2.namedWindow(win_name, cv2.WINDOW_NORMAL)
        cv2.resizeWindow(win_name, 800, 800)
        
        roi = cv2.selectROI(win_name, fft_norm, fromCenter=True)
        cv2.destroyWindow(win_name)
        x, y, w, h = [int(v) for v in roi]
    else:
        # Se as coordenadas foram passadas, apenas desempacotamos
        # Esperamos o formato (x, y, w, h)
        x, y, w, h = roi_coords

    # 3. Filtragem (igual ao que fizemos antes)
    mask = np.zeros((M, N), dtype=complex)
    mask[y:y+h, x:x+w] = 1
    filtered_fft = fft_holo * mask

    # 5. Visualização do filtro (se display=True)
    if display:
        # Usamos a sua função de intensidade já existente no script
        show_img = intensity(filtered_fft, True)
        # show_img = intensity(np.fft.ifftshift(filtered_fft), True)
        imageShow(show_img, 'Spatial filter (Interactive)')

    # final_field = np.fft.ifft2(np.fft.ifftshift(filtered_fft))
    final_field = np.fft.ifft2(np.fft.ifftshift(filtered_fft)).astype(complex)
    # final_field = np.fft.ifft2(filtered_fft)
    return final_field, x, y, w, h
