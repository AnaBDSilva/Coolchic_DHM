...
def angularSpectrum(field, z, wavelength, dx, dy):
    ...
    # phase = np.exp(2j * z * pi * np.sqrt(np.power(1 / wavelength, 2) - (np.power(X * dfx, 2) + np.power(Y * dfy, 2))))

    # 1. Calcular as frequências ao quadrado
    f2 = (X * dfx)**2 + (Y * dfy)**2

    # 2. Calcular kz garantindo que o array é complexo para evitar NaN
    term = (1/wavelength)**2 - f2
    kz = np.sqrt(term.astype(complex))

    # 3. Calcular a fase
    phase = np.exp(2j * pi * z * kz)
 ...
 """
def angular_spectrum(field, z, wavelength, dx, dy):
    """
    Angular spectrum propagation of a complex field.
    """
    field = np.asarray(field)
    M, N = field.shape

    # Frequency coordinates (more efficient than meshgrid of indices)
    fx = np.fft.fftfreq(N, d=dx)
    fy = np.fft.fftfreq(M, d=dy)
    FX, FY = np.meshgrid(fx, fy)

    # Wavenumber
    k = 1 / wavelength

    # Transfer function (handle evanescent waves safely)
    argument = k**2 - (FX**2 + FY**2)
    argument = np.maximum(argument, 0)  # avoid negative sqrt

    H = np.exp(1j * 2 * np.pi * z * np.sqrt(argument))

    # Forward FFT
    field_spec = np.fft.fft2(field)

    # Apply transfer function
    out_spec = field_spec * H

    # Inverse FFT
    out = np.fft.ifft2(out_spec)
