...
def holo_read(filename, vargin):
    ...
    # Read Sample Holo .mat file
    dataHolo = loadmat(filename)

    holo = dataHolo["I1"].squeeze()
...
def get_plus1(bw):
    ...
    rect = Rectangle((plus1[1], plus1[0]),  plus1[3] - plus1[1], plus1[2] - plus1[0], 
                     linewidth=2, edgecolor='r', facecolor='none', label='Auto BBox')
    
    plt.gca().add_patch(rect)
    plt.title(f'Imagem Binária - Ordem +1 Detectada\n(x={plus1[1]}, y={plus1[0]}, w={plus1[3] - plus1[1]}, h={plus1[2] - plus1[0]})')
    plt.colorbar(label='Intensidade Binária')
    plt.legend()
    plt.show()
    ###############
    ...
    return plus_coor, m, n, p, q, col_min, row_min, width, heigth
...
def fminunc(minfunc, Cy):
    ...
    gradient = lambda t: approx_fprime(minfunc, t)
...
def spatialFilterinCNT(inp, M, N):
    ...
    cv2.namedWindow('Selecione a ROI', cv2.WINDOW_NORMAL) 
    cv2.resizeWindow('Selecione a ROI', 800, 800)
    ROI = cv2.selectROI('Selecione a ROI', fft_holo_image, fromCenter=True)
...
def fast_CNT(inp, wavelength, dx, dy):
    ...
    cv2.namedWindow('image', cv2.WINDOW_NORMAL) # Adicione o cv2.WINDOW_NORMAL aqui
